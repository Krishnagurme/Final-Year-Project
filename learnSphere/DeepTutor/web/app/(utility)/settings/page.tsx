...
function getActiveProfile(catalog: Catalog, serviceName: ServiceName): CatalogProfile | null {
  if (!catalog.services?.[serviceName]?.profiles) return null
...
function getActiveModel(catalog: Catalog, serviceName: ServiceName): CatalogModel | null {
  ...
  if (!catalog.services?.[serviceName]) return null
  const service = catalog.services[serviceName]
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setLanguage('en')
        setStatus({
          backend: { status: 'offline', timestamp: '' },
          llm: { status: 'unknown' },
  ...
  const reopenTour = async () => {
    ...
    setToast(
      payload.command
        ...
        : payload.message || 'Run python scripts/start_tour.py in your terminal.'
    )
  }
              ...
              {(['llm', 'embedding', 'search'] as const).map(service => (
                ...
                >
                  ...
                  <span className="text-[11px] text-[var(--muted-foreground)]/60">
                    {draft.services?.[service]?.profiles?.length || 0}
                  </span>
                </button>
              ))}
            </div>
