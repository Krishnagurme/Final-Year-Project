          ...
          [action.key]: {
            ...
            messages: [
              ...(state.sessions[action.key]?.messages ?? []),
              {
                ...
                events: [],
                capability:
                  (state.sessions[action.key] ?? createSessionEntry(action.key)).activeCapability ||
                  '',
              },
            ],
            updatedAt: Date.now(),
  ...
  const sendThroughRunner = useCallback(
    function dispatchToRunner(key: string, msg: ChatMessage, attempt = 0) {
      ...
      if (!runner.client.connected) {
        if (attempt >= 50) {
          console.warn('WebSocket failed to connect after retries')
          dispatch({ type: 'STREAM_END', key, status: 'failed' })
          return
        }
        window.setTimeout(() => dispatchToRunner(key, msg, attempt + 1), 1000)
        return
      }
  ...
  const sendMessage = useCallback(
    (
      ...
      sendThroughRunner(key, {
        ...
        ...(historyReferences?.length ? { history_references: historyReferences } : {}),
