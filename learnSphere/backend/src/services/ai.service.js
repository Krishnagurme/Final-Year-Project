...
const getCompletionsUrl = () =>
  `${(process.env.AI_BASE_URL || process.env.OPENAI_BASE_URL || '').replace(/\/+$/, '')}/chat/completions`;
...
export const aiService = {
  ...
  async generateDynamicTest(subject, numberOfQuestions = 5) {
    ...
    try {
      ...
      const testData = parseJsonContent(content);
      return {
        success: true,
  ...
  async evaluatePrerequisites(answers, subject, courseLevel) {
    ...
    try {
      ...
      const content = response.data.choices[0].message.content;
      const evaluation = parseJsonContent(content);
  ...
  async generateLearningPath(studentProfile, courseId, assessmentResults) {
    try {
      ...
      const content = response.data.choices[0].message.content;
      const learningPath = parseJsonContent(content);
